...
async function sendEmail(to, subject, text) {
  ...
  await transporter.sendMail({ from: `"Khamsa Hotel" <${EMAIL_USER}>`, to, subject, text });
}
...
app.use((req, res, next) => {
  ...
  if (o && ALLOWED_ORIGINS.includes(o)) res.setHeader("Access-Control-Allow-Origin", o);
...
app.get("/health", (_req, res) => {
  res.status(200).json({ ok: true, pid: process.pid, port: process.env.PORT, time: new Date().toISOString() });
});
...
app.post("/api/bookings", async (req, res) => {
  try {
    const { checkIn, checkOutTime, duration, rooms, guests, firstName, lastName, phone, email, price } = req.body;
...
app.post("/api/bookings/commit", async (req, res) => {
  try {
    const { checkIn, duration, rooms, firstName, lastName, phone, email, guests, price, paymentId, shopTxId, comment } = req.body;
    ...
    try { bnovoResp = await bnovo.createBooking(bookingPayload); bnovoOk = true; }
    catch (e) { bnovoErr = e?.data || e?.message || e; console.error("❌ Bnovo createBooking xato:", bnovoErr); }
    ...
    if (!bnovoOk) return res.status(502).json({ success: false, error: "Bnovo'ga bron yuborilmadi", details: bnovoErr });
