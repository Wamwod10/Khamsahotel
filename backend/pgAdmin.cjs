  ...
  // ENV-dagi inline kommentlarni tozalash
  const clean = (v, def = "") => String(v ?? def).split("#")[0].trim();

  require("dotenv").config({ path: path.resolve(__dirname, ".env") });
  // const PORT = Number(process.env.PORT || 5004);
  const PORT = Number(process.env.PGADMIN_PORT || process.env.PORT || 5004);
  /* ---------- App ---------- */
  const app = express();
  ...
  // PG: DATE (OID 1082) -> 'YYYY-MM-DD' string
  ...
  const PG_CONFIG = {
    ...
    port: +clean(process.env.PGPORT, "5432"),
  ...
  // const PORT = +clean(process.env.PORT, "5004");

  console.log('ENV PORT=', process.env.PORT, '__dirname=', __dirname);
  ...
  /* ---------- Schema ensure (+ eski nomlardan migratsiya) ---------- */
  async function ensureSchema() {
    await pool.query(`CREATE TABLE IF NOT EXISTS public.khamsachekin (id SERIAL PRIMARY KEY);`);

    // Eski ustunlar bo‘lsa rename; bo‘lmasa yangi ustunlarni qo‘shish
    await pool.query(`
    DO $$
  ...
  /** Header.jsx uchun: berilgan kundan boshlab to‘g‘ri keladigan eng yaqin blokni qaytaradi */
